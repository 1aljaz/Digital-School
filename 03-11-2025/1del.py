# Napiši program, ki uporablja modul modul1.py in analiziraj besedilo. Besedilo je v datoteki
# besedilo.txt in vam ga ni treba spreminjati. Funkcija, ki besedilo odpira vam ni treba napisati.

besedilo = ""
with open("besedilo.txt", "r", encoding="utf-8") as f:
    for l in f:
        besedilo += l

print(besedilo)
# Napišite program, ki pove: 
# - najpogostejšo besedo v stavku,
# - najpogostejšo črko, 
# - najpogostejšo besedo, ko besedilo obrnete,
# - potem vse stavke v besedilo lepo kapitalizirajte, in ponovite vse prejšnje stvari.


# Poglejte si še, modul1.py, preberite opise funkcij.
